...
// Admin routes are now handled by the dedicated admin router
...
app.get(['/api/debug/data-sources', '/debug/data-sources'], async (req, res) => {
  const debugData = {
    timestamp: new Date().toISOString(),
    environment: {},
    clerk: {},
  ...
  try {
    ...
    debugData.environment = {
      ...
      buildsCollection: process.env.BUILDS_COLLECTION || 'Builds',
    ...
    try {
      ...
      debugData.errors.push(`Clerk init error: ${clerkError.message}`)
    }
    ...
    try {
      ...
      const collectionsToTest = [
        { name: debugData.environment.ordersCollection, key: 'orders' },
        { name: debugData.environment.buildsCollection, key: 'builds' },
      ...
      if (debugData.collections.builds?.count > 0) {
        const buildsCollection = db.collection(debugData.environment.buildsCollection)
      ...
      if (debugData.collections.orders?.count > 0) {
        const ordersCollection = db.collection(debugData.environment.ordersCollection)
