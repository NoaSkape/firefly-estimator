 ...
 * Generates complete HTML contract with DocuSeal field tags
...
/**
 * Build DocuSeal-compatible HTML template with proper HTML field elements
 * This version converts {{field;type=text;role=buyer}} syntax to HTML elements
 */
export function buildDeliveryHtmlForDocuSeal() {
...
<style>
  ...
  h2 { font-size: 14px; margin: 16px 0 6px; font-weight: bold; }
  .muted { color:#555; }
  .box { border:1px solid #aaa; border-radius:4px; padding:10px 12px; margin:8px 0; background-color: #f9f9f9; }
  .kv { margin:4px 0; }
  ...
  .label { width:160px; color:#333; }
  .line { flex:1; border-bottom:1px solid #333; height:18px; }
  .page-initials { margin-top:28px; text-align:center; font-size: 11px; }
  .small { font-size: 11px; }
...
<div class="box">
  <div class="kv"><span>Dealer (Seller):</span> Firefly Tiny Homes LLC</div>
  <div class="kv"><span>Dealer Email:</span> office@fireflytinyhomes.com</div>
  <div class="kv"><span>Buyer:</span> {{buyer_full_name;type=text;role=buyer;required=true}} ({{buyer_email;type=text;role=buyer;required=true}})</div>
  <div class="kv"><span>Co-Buyer (optional):</span> {{cobuyer_full_name;type=text;role=cobuyer;required=false}} ({{cobuyer_email;type=text;role=cobuyer;required=false}})</div>
</div>

<h2>UNIT & DELIVERY LOCATION</h2>
<div class="box">
  <div class="kv"><span>Brand / Model / Year:</span> {{model_brand;type=text;role=buyer;required=true}} / {{model_code;type=text;role=buyer;required=true}} / {{model_year;type=text;role=buyer;required=true}}</div>
  <div class="kv"><span>Dimensions:</span> {{dimensions;type=text;role=buyer;required=true}}</div>
  <div class="kv"><span>Delivery Address:</span> {{delivery_address;type=text;role=buyer;required=true}}</div>
  <div class="kv"><span>Estimated Factory Completion:</span> {{est_completion_date;type=text;role=buyer;required=true}}</div>
</div>
...
<div class="box">
  Buyer is solely responsible for preparing the site to accept the Unit. At minimum, Buyer confirms and initials each requirement:
  <ol class="list">
    <li>Legal access, permits, approvals, and compliance with local codes/zoning are secured by Buyer prior to delivery. {{site_initials_1;type=initials;role=buyer;required=true}}</li>
    <li>Approach route supports oversize transport: width, height, and turning radius are adequate; obstructions trimmed/removed. {{site_initials_2;type=initials;role=buyer;required=true}}</li>
    <li>Ground conditions along the route and pad are firm, level, and dry; no soft sand, mud, or saturated soil. {{site_initials_3;type=initials;role=buyer;required=true}}</li>
    <li>Pad or foundation prepared to manufacturer spec, level within tolerance, and accessible to delivery vehicle(s). {{site_initials_4;type=initials;role=buyer;required=true}}</li>
    <li>Driveways/culverts/bridges rated for vehicle weight; Buyer assumes responsibility for any damage to private improvements. {{site_initials_5;type=initials;role=buyer;required=true}}</li>
    <li>All utilities (power, water, sewer/septic) are stubbed-out within required distance and capped for safe connection. {{site_initials_6;type=initials;role=buyer;required=true}}</li>
  </ol>
</div>
...
<div class="box">
  <ul class="list">
    <li>Dealer will coordinate a target delivery window after factory completion. Buyer shall be present or represented by an authorized adult. {{delivery_initials_1;type=initials;role=buyer;required=true}}</li>
    <li>If route is impassable or unsafe on the scheduled day (weather, ground, obstacles), Dealer or carrier may postpone at their sole discretion. {{delivery_initials_2;type=initials;role=buyer;required=true}}</li>
    <li>Escort/pilot cars, permits, or police details required by jurisdiction are billable to Buyer and will be invoiced at cost plus administrative fee. {{delivery_initials_3;type=initials;role=buyer;required=true}}</li>
  </ul>
</div>
...
<div class="box">
  <ul class="list">
    <li>If delivery cannot be completed due to site conditions not meeting Section 1, Buyer agrees to pay reasonable standby charges and any redelivery fees quoted by the carrier. {{fees_initials_1;type=initials;role=buyer;required=true}}</li>
    <li>Any additional equipment required (e.g., skid steer, winch truck, crane) will be billed to Buyer at cost plus administrative fee. {{fees_initials_2;type=initials;role=buyer;required=true}}</li>
    <li>Changes to the planned set requiring new drawings, permits, or engineering are out-of-scope and billable. {{fees_initials_3;type=initials;role=buyer;required=true}}</li>
  </ul>
</div>
...
<div class="box">
  <ul class="list">
    <li><span class="conspicuous">Risk of loss passes to Buyer upon factory completion.</span> Dealer will reasonably assist with carrier claims but is not liable for transit damage. {{risk_initials_1;type=initials;role=buyer;required=true}}</li>
    <li>Minor transport scuffs and adjustments will be captured on a punch list and addressed per manufacturer policy; they do not constitute refusal of delivery. {{risk_initials_2;type=initials;role=buyer;required=true}}</li>
  </ul>
</div>

<h2>5) INSURANCE</h2>
<div class="box">
  Buyer shall maintain insurance coverage from the time of factory completion, naming Firefly Tiny Homes LLC as additional insured for the Purchase Price minimum until final payment clears. Proof of coverage may be requested prior to release. {{insurance_initials;type=initials;role=buyer;required=true}}
</div>

<h2>6) STORAGE AFTER COMPLETION</h2>
<div class="box">
  If delivery is delayed more than twelve (12) days after factory completion for reasons outside Dealer's control, storage fees of $50/day accrue and are due before release. {{storage_initials;type=initials;role=buyer;required=true}}
</div>

<h2>7) INDEMNIFICATION</h2>
<div class="box">
  Buyer agrees to defend, indemnify, and hold harmless Dealer, its employees, and carriers from claims, damages, or costs arising from site conditions, access routes, or Buyer's directions, except to the extent caused by Dealer's gross negligence or willful misconduct. {{indemnification_initials;type=initials;role=buyer;required=true}}
</div>
...
<div class="page-initials">
  Buyer Initials: {{buyer_page_initials;type=initials;role=buyer;required=true}} &nbsp;&nbsp; Co-Buyer Initials: {{cobuyer_page_initials;type=initials;role=cobuyer;required=false}}
</div>

<h2>SIGNATURES</h2>

<div style="margin: 20px 0;">
<strong>DEALER (FIREFLY TINY HOMES)</strong><br>
...
`;
  ...
  // Convert field tags to HTML elements for DocuSeal HTML API
  return convertFieldTagsToHtmlElements(htmlWithFieldTags)
}
