...
export async function getDeliveryQuote(destAddress, settings) {
  ...
  const rate = Number(settings?.pricing?.delivery_rate_per_mile || 0)
  const minimum = Number(settings?.pricing?.delivery_minimum || 0)
  ...
  const fee = Math.max(roundToCents(miles * rate), minimum)
