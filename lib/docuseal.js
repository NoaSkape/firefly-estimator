...
export async function createSubmission({ templateId, prefill, submitters, sendEmail = false, order = 'preserved', completedRedirectUrl, cancelRedirectUrl }) {
  ...
  // For template-based submissions, use 'values' field to prefill data
  // This ensures the data appears in the downloaded PDF
  const submittersWithValues = (submitters || []).map(submitter => ({
    ...submitter,
    values: prefill || {}  // Use 'values' field for template prefilling
  }))
  ...
  const body = {
    ...
    submitters: submittersWithValues,
  }
  ...
  console.log('[DOCUSEAL] Creating submission with:', {
    ...
    prefillValues: prefill ? Object.entries(prefill).slice(0, 5) : [], // Show first 5 values
    submittersCount: submittersWithValues.length,
    sendEmail,
    order,
  ...
  console.log('[DOCUSEAL] Prefill values being sent:', prefill ? Object.entries(prefill).map(([key, value]) => ({ field: key, value })) : [])
  console.log('[DOCUSEAL] Submitters with values:', JSON.stringify(submittersWithValues, null, 2))
