...
export function reprice(buildLike) {
  ...
  const options = Array.isArray(buildLike?.selections?.options)
    ...
    : 0
  // Default delivery and setup costs if not specified
  const delivery = Number(buildLike?.pricing?.delivery || 1500)
  ...
  // Debug log to see what's happening with base price
  console.log('reprice debug:', {
    base,
    options,
    delivery,
    setup,
    ...
    tax,
    total,
    selections: buildLike?.selections
  })
  
  return { base, options, subtotal, tax, delivery, setup, total }
}
...
export async function createBuild({ userId, modelSlug, modelName, basePrice, selections = {}, financing = {}, buyerInfo = {}, status = 'DRAFT' }) {
  ...
  const doc = {
    ...
    pricing: reprice({ selections: { basePrice, options: selections.options }, pricing: { delivery: 1500, setup: 500 } }),
    financing: {
