...
export async function generateOrderSummaryPDF(order) {
  ...
  try {
    ...
    // Fallback: Return a simple text-based PDF if Puppeteer fails
    console.warn('[PDF_GENERATOR] Falling back to simple text-based PDF')
    return generateFallbackPDF(order)
  }
}

// Fallback PDF generator for when Puppeteer fails
function generateFallbackPDF(order) {
  const content = `
ORDER SUMMARY
=============
