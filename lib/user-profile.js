...
export async function updateUserProfile(userId, profileData) {
  ...
  if (!result.value) {
    console.log('DEBUG: result.value is null/undefined, fetching profile manually')
    // Fallback: fetch the profile manually
    const fallbackProfile = await collection.findOne({ userId })
    console.log('DEBUG: fallback profile:', fallbackProfile)
...
export async function addUserAddress(userId, address) {
  ...
  if (addressWithMeta.isPrimary) {
    addresses.forEach((addr, index) => {
      // Make all addresses non-primary except the one we just updated/added
      if (existingIndex >= 0) {
        // We updated existing address - make all others non-primary except the one at existingIndex
        if (index !== existingIndex) {
          addr.isPrimary = false
        }
      } else {
        // We added new address - make all others non-primary except the last one (newly added)
...
/**
 * Update user's basic info (name, email, phone, and address)
 * @param {string} userId - Clerk user ID
...
export async function updateUserBasicInfo(userId, basicInfo) {
  ...
  const updatedProfile = await updateUserProfile(userId, {
    ...
    // Store address fields directly on profile (EXACTLY like phone)
    address: basicInfo.address || profile.address,
    city: basicInfo.city || profile.city,
    state: basicInfo.state || profile.state,
    zip: basicInfo.zip || profile.zip
  })
...
export async function getAutoFillData(userId) {
  ...
  // Use direct profile address fields (EXACTLY like phone) for immediate autofill
  // This ensures address works the same way as phone, name, email
  const directAddress = {
  ...
  // Fallback to primary address from addresses array if direct fields are empty
  // This maintains backward compatibility and advanced features
  const primaryAddress = await getPrimaryAddress(userId)
  const fallbackAddress = primaryAddress ? {
  ...
  // Build delivery address from whichever source has data
  const addressToUse = directAddress.address ? directAddress : fallbackAddress
  const deliveryAddress = addressToUse.address ? 
    [addressToUse.address, addressToUse.city, addressToUse.state, addressToUse.zip]
      .filter(Boolean)
      .join(', ') : ''
  ...
  return {
    ...
    email: profile.email || '',
    phone: profile.phone || '',
    // Use direct profile fields first (like phone), fallback to primary address
    address: directAddress.address || fallbackAddress.address,
    city: directAddress.city || fallbackAddress.city,
    state: directAddress.state || fallbackAddress.state,
    zip: directAddress.zip || fallbackAddress.zip,
    deliveryAddress
  }
}
