...
export async function ensureOrderIndexes() {
  ...
  await Promise.all([
    ...
    col.createIndex({ 'contract.submissionId': 1 }, { sparse: true }),
  ])
}
...
export async function setOrderPricingSnapshot(orderId, snapshot) {
  const db = await getDb()
  const col = db.collection(ORDERS_COLLECTION)
  const { ObjectId } = await import('mongodb')
  const _id = new ObjectId(String(orderId))
  await col.updateOne({ _id }, { $set: { pricingSnapshot: snapshot, updatedAt: new Date() } })
  return col.findOne({ _id })
}

export async function setOrderDelivery(orderId, delivery) {
  const db = await getDb()
  const col = db.collection(ORDERS_COLLECTION)
  const { ObjectId } = await import('mongodb')
  const _id = new ObjectId(String(orderId))
  await col.updateOne({ _id }, { $set: { delivery, updatedAt: new Date() } })
  return col.findOne({ _id })
}

export async function updateContract(orderId, updates) {
  const db = await getDb()
  const col = db.collection(ORDERS_COLLECTION)
  const { ObjectId } = await import('mongodb')
  const _id = new ObjectId(String(orderId))
  await col.updateOne({ _id }, { $set: { contract: { ...(updates || {}) }, updatedAt: new Date() } })
  return col.findOne({ _id })
}

export async function appendContractEvent(orderId, event) {
  const db = await getDb()
  const col = db.collection(ORDERS_COLLECTION)
  const { ObjectId } = await import('mongodb')
  const _id = new ObjectId(String(orderId))
  await col.updateOne({ _id }, { $push: { 'contract.events': event } })
}
