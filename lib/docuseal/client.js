...
export async function createPackEnvelope(orderId, pack, order) {
  ...
  const data = await response.json()
  console.log('[DOCUSEAL_CLIENT] Submission response:', JSON.stringify(data, null, 2))
  ...
  let signingUrl = ''
  let envelopeId = data.id || data.submission_id
  
  if (data.submitters && Array.isArray(data.submitters)) {
    const buyerSubmission = data.submitters.find(s => s.role === 'Buyer')
    signingUrl = buyerSubmission?.url || buyerSubmission?.signing_url
  } else if (data.url) {
    signingUrl = data.url
  ...
  if (!signingUrl) {
    console.error('[DOCUSEAL_CLIENT] No signing URL found in response:', data)
    throw new Error('No signing URL found in DocuSeal response')
  }

  console.log('[DOCUSEAL_CLIENT] Submission created successfully:', envelopeId, 'URL:', signingUrl)
