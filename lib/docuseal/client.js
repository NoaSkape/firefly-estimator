...
export async function dsCreateTemplateFromHtml(body) {
  ...
  // CRITICAL DEBUG: Log the actual HTML being sent
  console.log('[DocuSeal] HTML Content Preview (first 500 chars):', body.html?.substring(0, 500))
  console.log('[DocuSeal] Field tags found in HTML:', (body.html?.match(/\{\{[^}]+\}\}/g) || []).length)
  console.log('[DocuSeal] Template data keys:', Object.keys(body))
  console.log('[DocuSeal] Roles specified:', body.roles)
...
export async function createPackEnvelope(orderId, pack, order) {
    ...
    body: JSON.stringify({
      ...
      submitters: [
        ...
        ...(order.coBuyer ? [{
          ...
          role: 'cobuyer',
          order: 0  // Same order as buyer = parallel signing
        }] : []),
        {
          name: 'Firefly Tiny Homes',
          email: process.env.DOCUSEAL_FIREFLY_EMAIL || 'office@fireflytinyhomes.com',
          role: 'firefly_signer',
          order: 1  // Counter-signs after customer(s)
        }
      ],
      fields: (() => {
        const fields = buildPackFields(pack, order)
        console.log('[DOCUSEAL_CLIENT] Prefilling fields for pack:', pack, 'Fields:', Object.keys(fields).length, 'Sample:', {
          buyer_full_name: fields.buyer_full_name,
          buyer_email: fields.buyer_email,
          buyer_phone: fields.buyer_phone,
          model_brand: fields.model_brand,
          price_total: fields.price_total
        })
        return fields
      })()
    })
  ...
  // ROBUST: Get signing URL from first submitter (workflow entry point)
  // This works regardless of role names or DocuSeal's role interpretation
  ...
  if (firstSubmitter) {
    ...
    console.log('[DOCUSEAL_CLIENT] Using first submitter for signing:', {
      id: firstSubmitter.id,
      role: firstSubmitter.role,
      name: firstSubmitter.name,
      url: signingUrl
    })
  }
  ...
  if (!signingUrl) {
    ...
    throw new Error('No signing URL found in DocuSeal response')
  }
...
function buildPackFields(pack, order) {
  ...
  const baseFields = {
    ...
    // Enhanced buyer information for comprehensive prefill
    buyer_first_name: order.buyer.firstName,
    buyer_last_name: order.buyer.lastName,
    buyer_phone_number: order.buyer.phone,
    buyer_email_address: order.buyer.email,
    buyer_street_address: order.deliveryAddress?.line1 || '',
    buyer_city: order.deliveryAddress?.city || '',
    buyer_state: order.deliveryAddress?.state || '',
    buyer_zip: order.deliveryAddress?.zip || '',
    buyer_country: order.deliveryAddress?.country || 'USA',
    ...
    model_name: order.model.name || order.model.model,
    model_description: order.model.description || '',
    ...
    options_total: formatCurrency(order.pricing.options),
    delivery_cost: formatCurrency(order.pricing.delivery || 0),
    setup_fee: formatCurrency(order.pricing.setup || 0),
    title_fee: formatCurrency(order.pricing.titleFee || 0),
  ...
  if (order.coBuyer) {
    ...
    baseFields.cobuyer_first_name = order.coBuyer.firstName
    baseFields.cobuyer_last_name = order.coBuyer.lastName
    baseFields.cobuyer_phone = order.coBuyer.phone || order.buyer.phone
    baseFields.cobuyer_email_address = order.coBuyer.email || order.buyer.email
  }
