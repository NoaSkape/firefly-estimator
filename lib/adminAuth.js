...
try {
  ...
  console.error('Failed to initialize Clerk client in adminAuth:', error.message);
}
import { z } from 'zod'
...
export const ROLE_PERMISSIONS = {
  ...
  [ADMIN_ROLES.ADMIN]: [
    ...
    PERMISSIONS.INTEGRATIONS_VIEW, PERMISSIONS.INTEGRATIONS_EDIT,
    PERMISSIONS.SECURITY_VIEW,
    PERMISSIONS.WORKFLOWS_VIEW, PERMISSIONS.WORKFLOWS_EDIT,
  ...
  async validateAdminAccess(req, res, next) {
    try {
      ...
      if (typeof next !== 'function') {
        console.error('Admin access validation error: next is not a function', { type: typeof next });
        return res.status(500).json({ error: 'Middleware chain error' });
      }
...
// Create a properly bound middleware function to avoid context loss in Express
