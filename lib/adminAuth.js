...
class AdminAuth {
  ...
  async validateAdminAccess(req, res, next) {
    try {
      // Use the existing requireAuth function for authentication
      const auth = await requireAuth(req, res, true)
      
      if (!auth) return; // Response already sent by requireAuth
      
      // Check if user has any admin role
      const role = await this.getUserRole(auth.userId)
      ...
      req.adminUser = {
        userId: auth.userId,
        role,
        permissions: await this.getUserPermissions(auth.userId)
      }
