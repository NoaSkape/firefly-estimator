...
const STEP_ROUTES = {
  ...
  'Customize!': (buildId, build = null) => {
    ...
    // Use the actual build ID from build data if available, otherwise use the passed buildId
    const actualBuildId = build?._id || buildId
    if (actualBuildId && actualBuildId !== 'magnolia' && actualBuildId.length > 10) { // Ensure it's a real build ID, not a slug
      return `/customize/${build?.modelSlug || 'magnolia'}?buildId=${actualBuildId}`
    }
