...
export default function OtherPolicies() {
  ...
  useEffect(() => {
    async function loadPolicy() {
      try {
        const response = await fetch('/api/policies/other-policies')
        if (response.ok) {
  ...
  return (
    <div>
      <Helmet>
        <title>Other Policies - Firefly Tiny Homes</title>
        <meta name="description" content="Firefly Tiny Homes additional policies including purchase terms, delivery, refunds, and consumer rights." />
      </Helmet>

      <div className="max-w-4xl mx-auto py-12 px-4">
        <h1 className="text-4xl font-bold text-gray-100 mb-8">{policy?.title || 'Other Policies'}</h1>
