...
export default function FAQPage() {
  ...
  const loadPageContent = async () => {
    try {
      ...
      // Use the same fallback content as above
      setPageContent({
        pageId: 'faq',
        content: {
          title: 'Park Model Homes FAQ | Firefly Tiny Homes',
    ...
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <header className="mb-8">
          ...
          <p className="mt-3 text-gray-300" dangerouslySetInnerHTML={{ __html: content.headerDescription || 'Welcome to our FAQ section! Here\'s everything you need to know about park model homes, Champion Park Model Homes, and the advantages of working with Firefly Tiny Homes—your online dealership for better pricing, personalized service, and the future of park model home buying.' }} />
        </header>

        {content.faqSections && content.faqSections.map((section, sectionIndex) => (
          <Section key={sectionIndex} title={section.title}>
            {section.items && section.items.map((item, itemIndex) => (
              <QA key={itemIndex} q={item.question}>
